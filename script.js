setTimeout(function () {window.scrollTo(0, 0);},200); //scroll to top-left when page loads; timeout set to delay; othewise doesn't work

//colors for rgb mode
const colors = ['#ff6666', '#ff9966','#cccc00', '#33ff99', '#80dfff','#9999ff','#cc99ff' ]
let colorIndex =-1
...
let backgroundSelectedColor = "grey";
let borderSelectedColor = "white" ;
let paintSelectedColor = "rgb(90, 90, 90)"; 
...
document.addEventListener("click", e =>{
    ...
    if(e.target.id != "pixelSlider" && e.target.id != "pixelButton"){
        pixelSlider.style.display = "none";
    }

    if (e.target.id == "pixelButton"){clickPixelButton(e); return;}
    if (e.target.id == "backgroundColorButton"){clickBackgroundColorButton(e); return;}
    if (e.target.id == "borderColorButton"){clickBorderColorButton(e); return;}
    if (e.target.id == "paintColorButton"){clickPaintColorButton(e); return;}
    if (e.target.id == "rgbButton"){clickRgbButton(e);return;}
    if (e.target.id == "eraserButton"){clickEraserButton(e);return;}
    if (e.target.id == "clearButton"){clickClearButton(e);return;}
});
...
function clickRgbButton(){
    if (isRgbModeOn == false){
        isRgbModeOn = true;
        rgbButton.style.background = "linear-gradient(90deg,"+
            "rgba(255, 0, 0, 1) 0%,"+
            "rgba(255, 154, 0, 1) 10%,"+
